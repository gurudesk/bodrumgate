...
if (!function_exists('validateForCustomFieldFromApi')) {
    function validateForCustomFieldFromApi($moduleName, $request, $categoryId = null)
    {
        ...
        if (!empty($categoryId)) {
            // $customizeHeaders = CustomizeUi::where('module_name', $moduleName)
            //                                 ->where('category_id', $categoryId)
            //                                 ->whereNull('category_id')
            //                                 ->get();
            $customizeHeaders = CustomizeUi::where(['module_name' => $moduleName, 'category_id' => null])
...
if (!function_exists('redirectView')) {
    function redirectView($routeName, $msg=null, $flag = "success", $parameter = null)
    {
...
if(!function_exists('deleteDataOfClientCustomFields')){
    function deleteDataOfClientCustomFields($keyword){
        ...
        $customizeUiDetailIds = CustomizeUiDetail::where("core_keys_id", "not like", "%".$keyword."%")->get()->pluck("id");
        CustomizeUiDetail::destroy($customizeUiDetailIds);
    }
...
if(!function_exists("updateTableIds")){
    function updateTableIds($oldAndNewTableIdArr)
    {
        foreach($oldAndNewTableIdArr as $oldAndNewTableIdObj){
            ...
            $oldCoreFieldArr = CoreFieldFilterSetting::where("table_id", $oldAndNewTableIdObj['oldTableId'])->get();
            ...
            $oldCustomFieldArr = CustomizeUi::where("table_id", $oldAndNewTableIdObj['oldTableId'])->get();
...
if(!function_exists("updateCustomFields")){
    function updateCustomFields($oldAndNewCoreKeysIdLangKeyArr)
    {
        ...
        foreach($oldAndNewCoreKeysIdLangKeyArr as $oldAndNewCoreKeysIdLangKeyObj){
            ...
            $customizeUi = CustomizeUi::where("core_keys_id", $oldCoreKeysId)->first();
