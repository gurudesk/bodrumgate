<template>
    ...
    <ps-layout>
        ...
        <div v-if="this.payment.id != this.offlinePaymentId && this.payment.id != this.promotionIAPPaymentId && this.payment.id != this.packageIAPPaymentId && this.payment.id != this.vendorSubscriptoinPlanPaymentId">
                    ...
                    <ps-data-table :rows="this.paymentCoreKeys" :columns="columns" searchHide="true">
                        <template #tableActionRow="rowProps">
                            ...
                            <div class="flex flex-row mb-2" v-if="rowProps.field == 'action'">
                                <ps-button @click="handleEdit(this.payment.id, rowProps.row.core_keys_id)" class="me-2" colors="bg-green-400 text-white"
...
export default defineComponent({
    ...
    layout: PsLayout,
    props: ['errors', 'payment', 'paymentCoreKeys', 'offlinePaymentId', 'promotionIAPPaymentId', 'packageIAPPaymentId', 'vendorSubscriptoinPlanPaymentId'],
    data() {
        return {
            form: useForm({
                name: "",
