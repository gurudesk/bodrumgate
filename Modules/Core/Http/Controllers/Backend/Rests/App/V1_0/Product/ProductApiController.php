...
class ProductApiController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        if (isset($entryForCoreAndCustom['createAsVendor']) && count($entryForCoreAndCustom['createAsVendor']) > 0) {
            $createAsVendor = VendorForItemEntryApiResource::collection($this->vendorService->getVendorByIds(['vendorCurrency'], $entryForCoreAndCustom['createAsVendor']));
        } else {
            $createAsVendor = [];
        }
    ...
    public function store(Request $request)
    {
        ...
        $errors = validateForCustomFieldFromApi($this->code, $request->product_relation, $request->category_id);
        ...
        if (empty($request->vendor_id) && isset($errors['ps-itm00046'])) {
            unset($errors["ps-itm00046"]);
        }
