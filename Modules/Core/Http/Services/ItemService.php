...
use Modules\StoreFront\VendorPanel\Http\Services\CartItemService;
...
class  ItemService extends PsService
{
    protected $fromHomePageSearch, $notFromHomePageSearch, $customUiDetailCoreKeysIdCol, $itemApiRelation, $loginUserIdParaApi, $deviceTokenParaApi, $okStatusCode, $customUiIdCol, $itmIdCol, $paginationPerPage, $pushNotificationTokenService, $userAccessApiTokenService, $itmStatusCol, $itmPurchasedOption, $itmDealOption, $itmConditionOfItem, $itmItemType, $soldOutItemReportCsvFileName, $successfulDealCountReportCsvFileName, $itemReportCsvFileName, $backendSettingService, $categoryType, $itemType, $userType, $allType, $paidItemFirstWithGoogleAdType, $bumbsAndGoogleAdsBetweenAdType, $onlyPaidItemAdType, $normalAdsOnlyAdType, $paidItemFirstAdType, $googleAdsBetweenAdType, $bumpsUpsBetweenAdType, $searchHistoryService, $searchHistoryItemType, $searchHistoryUserType, $searchHistoryCategoryType, $userInfoService, $CoreFieldFilterSettingIsDelCol, $internalServerErrorStatusCode, $successStatus, $noContentStatusCode, $dropDownUi, $textUi, $radioUi, $checkBoxUi, $dateTimeUi, $numberUi, $textAreaUi, $multiSelectUi, $imageUi, $timeOnlyUi, $dateOnlyUi, $customUiModuleName, $userService, $unBan, $imageService, $currencyService, $locationTownshipService, $locationCityService, $publish, $subcategoryService, $categoryService, $currencyCurrencyShortFormCol, $itmShopIdCol, $itmItemCurrencyIdCol, $itmItemLocationTownshipIdCol, $shopNameCol, $locationTownshipNameCol, $locationCityNameCol, $subCatNameCol, $itmItemLocationIdCol, $itmSubCatIdCol, $shopTableName, $currencyTableName, $locationTownshipTableName, $locationCityTableName, $subCatTableName, $uploadPathForDel, $thumb1xPathForDel, $thumb2xPathForDel, $thumb3xPathForDel, $delete, $unDelete, $enable, $disable, $hide, $show, $thumbnail3xPath, $thumbnail2xPath, $thumbnail1xPath, $originPath, $componentName, $customUiMandatoryCol, $itmInfoCoreKeysIdCol, $customUiIsDelCol, $screenDisplayUiIsShowCol, $catNameCol, $userNameCol, $itmCatIdCol, $itmUserIdCol, $screenDisplayUiIdCol, $coreFieldFilterIdCol, $screenDisplayUiKeyCol, $coreFieldFilterModuleNameCol, $customUiEnableCol, $itmInfoItemIdCol, $customUiCoreKeysIdCol, $itmTableName, $userTableName, $catTableName, $code, $parentPath, $getImgPath, $coreFieldFilterForRelation, $viewAnyAbility, $createAbility, $editAbility, $deleteAbility, $itmIsPaidCol, $itmAddedUserIdCol, $createdStatusCode, $forbiddenStatusCode, $badRequestStatusCode, $normalUserRoleId, $usrRemainingPost, $shopService, $approvalItem, $rejectItem, $disableItem, $publishItem, $successFlag, $dangerFlag, $warningFlag, $blockUserService, $systemConfigService, $searchHistoryAllType, $unpublishItem, $vendorService,
        $itmIsSoldOutCol, $itmIsDiscountCol, $itmIsAvailableCol, $itmOverallRatingCol, $itmPriceCol, $itmTitleCol, $itmDescriptionCol, $itmLatCol, $itmLngCol, $paidItmTableName, $paidItmItemIdCol, $paidItmStartDateCol, $paidItmEndDateCol, $itmInfoTableName, $itmInfoValueCol, $googleAd, $itmSearchterm, $approvalNotiFlag, $subCatSubscribeService, $cartItemService;

    public function __construct(UserAccessApiTokenService $userAccessApiTokenService, PushNotificationTokenService $pushNotificationTokenService, BackendSettingService $backendSettingService, SearchHistoryService $searchHistoryService, UserService $userService, ImageService $imageService, CurrencyService $currencyService, LocationTownshipService $locationTownshipService, LocationCityService $locationCityService, CategoryService $categoryService, SubcategoryService $subcategoryService, ShopService $shopService, BlockUserService $blockUserService, SystemConfigService $systemConfigService, UserInfoService $userInfoService, SubCatSubscribeService $subCatSubscribeService, VendorService $vendorService, CartItemService $cartItemService)
    {
        ...
        $this->mbSubscribeNotiFlag = Constants::mbSubscribeNotiFlag;
            ...
            foreach ($coreKeysIds as $coreKeysId) {
                foreach ($request->product_relation as $key => $value) {
                    ...
                    if ($coreKeysIdFromReq === $coreKeysId) {
                        ...
                        // if qty will null, will save as zero start
                        if($coreKeysId == Constants::itemQty){
                            if (empty($valueFromReq)) {
                                $itemRelation->value = 0;
                            }
                        }
                        // if qty will null, will save as zero end
    ...
    public function getCustomizeFields($code = null, $dataWithRelation = null, $coreKeysId = null, $isDel = null, $limit = null, $offset = null, $categoryId = null)
    {
        $customizeHeader  =  CustomizeUi::when($dataWithRelation, function ($q, $dataWithRelation) {
            ...
            ->when($categoryId !== null, function ($q) use ($categoryId) {
                if ($categoryId == "0") {
                    ...
                    $q->where('category_id', $categoryId)->orWhere('category_id', null);
                }
            })
            ->when($code, function ($q, $code) {
                $q->where($this->customUiModuleName, $code);
            })
            ->when($isDel !== null,  function ($q) use ($isDel) { //check is_delete twice
                $q->where($this->customUiIsDelCol, $isDel);
            })
            ->when($limit, function ($query, $limit) {
                $query->limit($limit);
    ...
    public function searching($query, $conds)
    {
        ...
        if (isset($conds['vendor_id']) && $conds['vendor_id']) {
            $query->where($this->itmTableName . '.' . Item::vendorId, $conds['vendor_id']);
        }
    ...
    {
        $qtyCoreKyesId = 'ps-itm00046';
        $quantity = ItemInfo::where([ItemInfo::itemId => $itemId, ItemInfo::coreKeysId => $qtyCoreKyesId])->first();
    ...
    public function controlFieldArr($status = null)
    {
        // for control
        $controlFieldArr = [];
    ...
    public function takingForColumnAndFilterOption($status = null, $categoryId = null)
    {
        ...
        foreach ($hideShowForCoreAndCustomFields as $showFields) {
            ...
            if ($showFields->customizeField !== null && ($showFields->customizeField->category_id == $categoryId || $showFields->customizeField->category_id == null)) {
    ...
    public function index($request)
    {
        ...
        foreach($categoriesWithCount as $category){
            ...
            $category->count = $this->countCategory($category->id);
        }

        // check permission
        $checkPermission = $this->checkPermission($this->viewAnyAbility, Item::class, "admin.index");
        $response["checkPermission"] = $checkPermission;
    ...
    public function getItemList($request)
    {
        ...
        // dd($request->input('vendor_id'), $request->vendor_id);
        $conds['vendor_id'] = $request->input('vendor_id') ?? $request->vendor_id ?? '';
        ...
        $columnAndColumnFilter = takingForColumnAndFilterOption($this->code, $this->controlFieldArr("item"));

        // $columnAndColumnFilter = $this->takingForColumnAndFilterOption('item', $categoryId);
        ...
        $selected_category = new CategoryWithKeyResource($this->categoryService->getCategory($conds['category_id']));
        ...
        // dd($columnAndColumnFilter[ps_constant::handlingColumn], $columnAndColumnFilter[ps_constant::handlingFilter]);
        if ($conds['order_by']) {
            $dataArr = [
    ...
    public function countCategory($categoryId = null, $conds = null)
    {
        if ($categoryId == null) {
            ...
            $count = Item::where($this->itmCatIdCol, $categoryId)
                ->whereIn($this->itmStatusCol, [1, 4])
                ->when($conds, function ($query, $conds) {
                    $query->where($conds);
                })
                ->count();
    ...
    public function create($routeName, $request)
    {
        ...
        $customizeHeader = $this->getCustomizeFields($code, null, null, 0, null, null, $request->category_id);
        $customizeDetail = $this->getCustomizeFieldAttrs();
        $categories = CategoryWithKeyResource::collection($this->categoryService->getCategories(null, $this->publish));
        $subCategories = $this->subcategoryService->getSubCategories(null, $this->publish);
    ...
    public function edit($id, $routeName)
    {
        ...
        $coreFieldFilterSettings = $this->getCoreFieldFilteredLists($code);
        $customizeHeader = $this->getCustomizeFields($code, null, null, 0, null, null, $item['category_id']);
        // $customizeHeader = $this->getCustomizeFields($code, null, null, 0);
        $customizeDetail = $this->getCustomizeFieldAttrs();
    ...
    public function destroy($id, $routeName = null)
    {
        ...
        $cartItem = $this->cartItemService->getCartItem(itemId:$id);
        if(!empty($cartItem )){
            $cartItem->delete();
        }
    ...
    public function soldOutItemReportShow($id, $relations)
    {
        ...
        $cityRelation = ['township'];
        $itemRelation = [
            'vendor', 'category', 'subcategory', 'psx_user_boughts', 'city', 'township', 'currency', 'owner',
    ...
    public function getByIdFromApi($request)
    {
        ...
        $itemApiRelation = ['vendor', 'category', 'subcategory', 'city', 'township', 'currency', 'owner', 'itemRelation' => ['customizeUi'], 'cover', 'video', 'icon'];
    ...
    public function createFromApi($request)
    {
        ...
        $createAsVendor = haveVendorAndCreateAccess($request->login_user_id);

        $customizeUiRelation = ['uiTypeId'];
        $categoryId = $request->category_id;
        $custom = $this->getCustomizeFields($code, $customizeUiRelation, null, $this->unDelete, $limit, $offset, $categoryId);
        ...
        return [
            "custom" => $custom,
            "core" => $core,
            'createAsVendor' => !$createAsVendor ? [] : $createAsVendor
        ];
    }
    ...
    public function storeFromApi($request, $moduleName)
    {
        ...
        $systemConfig = $this->systemConfigService->getSystemConfig();
        // echo('hi');
        // check vendor expired? start
        $vendorId = $request->vendor_id;
        if (!empty($vendorId)) {
            $vendor = $this->vendorService->getVendor($vendorId);
            $isExpired = Carbon::now()->gt($vendor->expired_date);
            if ($isExpired) {
                $msg = __('core__api_vendor_expired', [], $request->language_symbol);
                return ['error' =>  $msg, 'status' =>  $this->forbiddenStatusCode];
            }
            if ($vendor->currency_id == null) {
                $msg = __('core__api_vendor_currency_error', [], $request->language_symbol);
                return ['error' =>  $msg, 'status' =>  $this->badRequestStatusCode];
            }
        }
        // check vendor expired? end
        ...
        if ($systemConfig->is_paid_app == 1) {
            ...
            if (empty($userInfo) || $userInfo->value == 0) {
                ...
                if ($request->vendor_id != '' && $request->vendor_id != null && !vendorPermissionControl(Constants::vendorItemModule, ps_constant::createPermission, $request->vendor_id)) {
                    $msg = __('core__api_update_no_permission_for_vendor', [], $request->language_symbol);
                    return ['error' =>  $msg, 'status' =>  $this->forbiddenStatusCode];
                }
        ...
        if ($status == $this->publishItem && $systemConfig->is_sub_subscription == 1) {
    ...
    public function soldOutFromItemDetailFromApi($request)
    {
        ...
        $this->decreaseItemQuantity($request->item_id, true);
    ...
    public function sendSubscribeNoti($item)
    {
        ...
        if ($updated_flag == 0) {
            ...
            if(!$isSubscribed){
                $isSubscribed = $this->subCatSubscribeService->getSubscribedSubCat($subcategory_id . $this->mbSubscribeNotiFlag);
            }
            // dd($isSubscribed);
