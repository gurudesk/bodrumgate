<template>
    ...
    <ps-layout>
        ...
        <ps-table2 ::row="row" :search="search" :object="this.items" :colFilterOptions="colFilterOptions" :columns="columns"
            :sort_field="sort_field" :sort_order="sort_order" :globalSearchPlaceholder="$t('core__be_search')"
            ...
            <template #searchLeft>
                <a :href="route('item_report.csv.export')"
            ...
            <template #Filter>
                ...
                <ps-dropdown v-if="$page.props.isSubCategoryOn == '1'" @on-click="subCategoryDropdownClick" class=" h-10">
                    ...
                    <template #filter>
                        <div class="mt-1 mx-1">
                            <ps-input-with-right-icon class="w-full h-10" theme="bg-gray-100" rounded="rounded-lg"
                                v-model:value="subCatSearch" :placeholder="$t('core__be_search')">
                                <template #icon>
                                    <ps-icon name="search" class='cursor-pointer' />
                                </template>
                            </ps-input-with-right-icon>
            ...
            <template #tableRow="rowProps">
                ...
                <span v-if="rowProps.field == itmConditionOfItem + '@@name'">
                    <ps-badge theme="text-red-600 bg-red-50" class="" v-if="rowProps.row[itmConditionOfItem + '@@name']">{{
                        rowProps.row[itmConditionOfItem + '@@name'] }}</ps-badge>
                </span>

                <!-- <span v-if="rowProps.field == price + '@@name'">
                    <ps-badge theme="text-red-600 bg-red-50" class="" v-if="rowProps.row[itmConditionOfItem + '@@name']">{{
                        rowProps.row[itmConditionOfItem + '@@name'] }}</ps-badge>
                </span> -->
...
export default defineComponent({
    ...
    setup(props) {
        ...
        if (props.selected_price_type != PsConst.NO_PRICE) {
            ...
            const indexOfTitleColumn = columns.findIndex(column => column.field === "subcategory_id@@name");
            const priceColumn = {
        ...
        function handleSearchingSorting(page = null, row = null) {
            router.get(route('item_report.index'),
                {
    ...
    methods: {
        ...
        FilterOptionshandle(value) {
            router.put(route('item_report.screenDisplayUiSetting.store'),
                {
                    value,
