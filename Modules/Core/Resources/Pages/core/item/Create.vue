    ...
    <ps-layout>
        ...
        $t('core__be_add_item_lbl') }}
                ...
                <div class="w-full px-4 mt-4 grid xxl:grid-cols-2 xl:grid-cols-2 lg:grid-cols-2 md:grid-cols-2 sm:grid-cols-1 gap-12"
                    v-if="vendorSetting && vendor_list.length > 0 && form.category_id != ''">
                    <div class='flex flex-col w-full mb-8 mt-2'>
                        <ps-label-header-6 class=" mt-4" textColor="text-secondary-800 dark:text-secondary-100">{{
        ...
        $t('core_be__always_choose_this_profile') }}</ps-label>
                ...
                <!-- card header end -->
                <ps-label-header-6 class="px-4 mt-4" textColor="text-secondary-800 dark:text-secondary-100">{{
        $t('category_selection_prioritize') }}
                </ps-label-header-6>
                <!-- for category dropdown -->
                <div
                    class="w-full px-4 mt-4 grid xxl:grid-cols-2 xl:grid-cols-2 lg:grid-cols-2 md:grid-cols-2 sm:grid-cols-1 gap-12">
                    <div class="w-full "
        ...
        product_relation_errors[custom_field_header.core_keys_id]
    }}</ps-label-caption>
                                    </div>
                                </div>
                                <!-- quantity -->
    ...
    form.location_township_id = form.location_township_id ? (townships.filter((township) => township.location_city_id == city.id).filter((township) => township.id == form.location_township_id)[0] ? form.location_township_id : '') : '']">
        ...
        product_relation_errors[custom_field_header.core_keys_id]
                                ...
                                <div class="mb-6" v-else>
                                    <open-street-map-pin-picker :dir="$page.props.dir" :lat="16.7967129"
                                        :lng="96.1609916" :onChange="setCoordinates" class="h-[500px]" />
                                </div>
                                <!-- maps end -->
