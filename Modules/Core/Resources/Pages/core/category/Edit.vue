                        ...
                        <div class="grid w-full sm:w-1/2 gap-6">
                            <div>
                                ...
                                <ps-input-with-right-icon theme="text-secondary-500" rounded="rounded" :placeholder="$t('core__category_placeholder')"
                                    ...
                                    <template #icon>
                                        <ps-icon name="editPencil" w="20" h="20" @click="categoryClick()"/>
                                    </template>
...
export default defineComponent({
    ...
    layout: PsLayout,
    props: ['errors', 'category','validation', 'languages', 'categoryLanguages'],
    setup(props) {
        ...
        const nameString = ref(trans(props.category.name));
        setTimeout(() => {
            //for page reload
            nameString.value = trans(props.category.name);
        }, 3000);
        ...
        function categoryClick() {
            if(language_update_modal_name.value.languageStrings.data.length == 0){
                props.languages.forEach(language => {
                    let languageStringObj = {};
                    languageStringObj.language = language;
                    languageStringObj.key = '_name';
                    languageStringObj.value = props.categoryLanguages.filter(lang => lang.language_id == language.id)[0]?.value;
                    languageStringObj.language_id = language.id;

                    language_update_modal_name.value.languageStrings.data.push(languageStringObj);
            ...
            language_update_modal_name.value.openModal('_name', (form) => {
        ...
        return { replaceIconClicked, validateNameInput, handleSubmit, ps_action_modal, form, loading, success, replaceImageClicked, ps_danger_dialog, ps_image_icon_modal, language_update_modal_name, categoryClick, nameString }
    },
    computed: {
        breadcrumb() {
            return [
