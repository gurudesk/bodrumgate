<template>
    ...
    <ps-layout>
        ...
        class="text-white mb-5 sm:mb-6 lg:mb-8"
         iconColor="white">{{status.msg.msg}}</ps-banner-icon>
        <!-- alert banner end -->
        ...
        @handleSort="handleSorting"  @handleSearch="handleSearching" @handleRow="handleRow"
            ...
            <template #searchLeft>
                <!-- <ps-button v-if="can.createMobileLanguageString" rounded="rounded" @click="jsonUploadClicked" class="ms-3 flex flex-wrap items-center ">
        ...
        function jsonUploadClicked(){
            ps_json_modal.value.openModal(
                (selectedFile) => {
                    ...
                    router.post(route('mobile_language_string.import.json', props.mobile_language.id), form);
                }
            );
        }
