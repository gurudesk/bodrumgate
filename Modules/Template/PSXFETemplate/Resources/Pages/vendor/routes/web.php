...
Route::middleware(["isFeSettingEnable"])->prefix('')->controller(MPCTemplate1Controller::class)->group(function (){
    Route::get('checkVendor', 'checkVendor');
    ...
    Route::get('/data-deletion-request-form', 'dataDeletionRequestForm')->name('fe_data_delete');
    ...
    Route::middleware(["isVendorSettingOn"])->group(function () {
        ...
        Route::get('/add-to-cart', 'feAddToCart')->name('fe_add_to_cart');
        Route::post('/add-to-cart', 'feAddToCart')->name('fe_add_to_cart');
        Route::get('/shopping-cart', 'feShoppingCart')->name('fe_shopping_cart');
        Route::get('/order-successful', 'feOrderSuccessful')->name('fe_order_successful');
        Route::get('/order-history', 'feOrderHistory')->name('fe_order_history');
        Route::get('/order-history-detail', 'feOrderHistoryDetail')->name('fe_order_history_detail');
    });
