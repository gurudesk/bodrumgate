    ...
    <ps-content-container>
        <template #content>
            <div class="mt-32 w-full">
                <ps-breadcrumb-2 :items="breadcrumb" class="mb-8" />

                <ps-label textColor="text-2xl font-semibold text-feAchromatic-800 dark:text-feAchromatic-50">{{ $t('core_fe__order_details') }}</ps-label>
                <!-- Order Summary -->
                <div class="flex flex-col px-6 py-4 rounded-lg border dark:border-feAchromatic-700 mt-6">
                    ...
                    <div class="flex flex-col md:flex-row lg:flex-row justify-between w-full">
                        <div class="w-full md:w-1/2 lg:w-1/2 flex flex-col justify-start">
                            ...
                            <div class="flex flex-row mb-4">
                                <ps-label class="w-1/2" textColor="text-feAchromatic-600 dark:text-feAchromatic-400"> {{ $t('order_status') }} </ps-label>
                                <div class="w-50 px-3 rounded" :style="'border:1px solid '+orderSummary?.data?.orderSummary?.orderStatusColor">
                                    <ps-label class="w-1/2" :style="'color:'+orderSummary?.data?.orderSummary?.orderStatusColor"> {{orderSummary?.data?.orderSummary?.orderStatus}} </ps-label>
                                </div>
                            </div>
                        ...
                        <div class="w-full md:w-1/2 lg:w-1/2 flex flex-col justify-start">
                            ...
                            <div class="flex flex-row mb-4">
                                ...
                                <div class="w-50 px-3 rounded" :style="'background-color:'+orderSummary?.data?.orderSummary?.paymentStatusColor+'20'">
                                    <ps-label class="w-1/2" :style="'color:'+orderSummary?.data?.orderSummary?.paymentStatusColor"> {{orderSummary?.data?.orderSummary?.paymentStatus}} </ps-label>
                                </div>
                            </div>
...
import OrderSuccessTitleAndValue from '@template1/vendor/views/vendor_checkout/components/OrderSuccessTitleAndValue.vue';
...
export default {
    ...
    setup(props) {
        ...
        console.log(props.query.orderId)
        ...
        breadcrumb() {
            return [
                ...
                },
                {
                    ...
                    url: route('fe_order_history')
                },
                {
                    label: trans('core_fe__order_details'),
