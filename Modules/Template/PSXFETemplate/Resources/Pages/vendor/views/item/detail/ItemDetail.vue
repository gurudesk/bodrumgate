                ...
                <div class="grid grid-cols-1 sm:grid-cols-12 gap-4">
                    <div class="col-span-1 sm:col-span-7 md:col-span-8">
                        ...
                        <item-detail-info-card
                            :productRelation="productRelation" :isSoldOutFeatureSettingOn="appInfoProvider?.isSoldOutFeatureSettingOn()" />
...
//import { useAddToCartStore } from '@templateCore/store/modules/item/AddToCartStore';
...
export default {
    ...
    setup(props) {
      ...
      //  const AddToCartStore = useAddToCartStore('detail');
        ...
        async function loadDataForItemDetail() {
            ...
            // Stock Quantity Notifiaction
            isOwner.value = productStore.item?.data?.user?.userId == loginUserId.value ? true : false;
            isUploadedByVendor.value = productStore.item?.data?.vendor?.id == '' ? false : true;
            for(const pr of productStore.item?.data?.productRelation){
                hasQty.value = pr.coreKeysId == 'ps-itm00046' ? true : false;
            }
