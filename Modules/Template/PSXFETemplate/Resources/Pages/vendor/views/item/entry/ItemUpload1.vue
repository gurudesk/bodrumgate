                        ...
                        <div class="pt-2 z-30">
                            <div >
                                <div
                                    ...
                                    @click="chooseProfile(userStore.user?.data?.userId, userStore.user?.data, 'user')">
                                    <img
                                ...
                                <div v-for="vendor in customFieldStore.customField.data?.vendorList"
                                    :key="vendor.id"
                                    class="w-full flex py-4 px-2 hover:bg-fePrimary-50 dark:hover:bg-fePrimary-900 cursor-pointer items-center"
                                    @click="chooseProfile(vendor.id, vendor, 'vendor')">
        ...
        <div class="w-full grid  grid-cols-12 gap-4 sm:gap-3.5 lg:gap-4 ">
            ...
            <div class="col-span-12  mt-4  lg:px-8  md:px-8 md:col-start-1 sm:col-start-2 px-6 w-full">
                <div class=" w-full">
                        ...
                        <div v-if="appInfoStore.appInfo.data?.psAppSetting?.SelectedPriceType == 'NORMAL_PRICE'" class="flex flex-row items-center  mt-2">
                            <div class=""
                                ...
                                <ps-dropdown align="left" class='  w-full' @onClick="loadCurrency">
                                    <template #select>
                                        <ps-dropdown-select :showCenter="true" :disabled="isVendorSelected"
                    ...
                    <!-- percent -->
                    <div v-if="appInfoStore.appInfo.data?.psAppSetting?.SelectedPriceType != 'NO_PRICE' && appInfoStore.appInfo.data?.mobileSetting?.is_show_discount == '1'">
                        <div class=" mb-4"
...
export default {
    ...
    setup(props) {
        ...
        function chooseProfile(id, profile, type){
            defaultProfileId.value = id;
            defaultProfileType.value = type;

            if(type == 'vendor'){
                paramHolder1.value.vendorId = id;
                paramHolder1.value.vendorName = profile.name;
                isVendorSelected.value = true;
                vendorCurrencyId.value = profile.currencyId;
                // paramHolder1.value.currencyShortForm = itemCurrencyStore?.itemCurrencyList?.data?.filter((list)=>list.id==vendorCurrencyId.value)[0].currencySymbol;
                paramHolder1.value.itemCurrencyId = profile.currencyId == "" ? defaultitemCurrencyStore.itemCurrencyList.data[0].id : profile.currencyId;
                paramHolder1.value.currencyShortForm = profile.currencySymbol == "" ? defaultitemCurrencyStore.itemCurrencyList.data[0].currencySymbol : profile.currencySymbol;
                validation.value.itemCurrencyStatus = false;
            }else{
                paramHolder1.value.vendorId = '';
                paramHolder1.value.vendorName = profile.userName;
                isVendorSelected.value = false;
