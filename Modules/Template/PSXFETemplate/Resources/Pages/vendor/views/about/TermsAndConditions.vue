<template>
    <Head :title="$t('term_and_condition__term_and_condition')" />
    <ps-content-container>
        <template #content>
            <div class='sm:mt-32 lg:mt-36 mt-28 mb-16 '>
                <ps-label class="text-center font-semibold text-2xl sm:text-3xl"> {{ $t("term_and_condition__term_and_condition") }} </ps-label>
                <ps-label class="mt-4" v-if="aboutUsStore.aboutus.data != null">
                   <span class="ck-content" v-html="aboutUsStore.aboutus.data.termsAndConditions" v-once></span>
                </ps-label>
