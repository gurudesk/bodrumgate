<template>
    <div class="w-full h-full" v-on:click="onClick != null ? onClick(vendor) : null">
        ...
        <ps-card class="border flex w-full bg-feAchromatic-50 flex-col lg:rounded-lg shadow-sm px-4 py-6 rounded items-center" :enabledHover="true">
            <div class="w-20 h-20 relative rounded-full overflow-hidden">
                ...
                <ps-lazy-image
                    ...
                    :src="$page.props.thumb2xUrl + '/' + vendor?.logo.imgPath"
                    :srcPlaceholder="$page.props.thumb1xUrl + '/' + vendor?.logo.imgPath"
                    :error="$page.props.sysImageUrl + '/default_vendor_shop.png'"
                />
            ...
            <div class="h-14 flex flex-col items-center justify-center">
                ...
                <div v-if="vendor?.expireStatus == PsConst.VendorExpiryExpiredStatus && vendor?.ownerUserId == loginUserId">
        ...
        <vendor-pending-vendor-modal v-if="showVendorPendingModal" ref="vendor_pending_vendor_modal"/>
        <vendor-application-modal v-if="showVendorApplicationModal" :vendorId="vendor.id" ref="vendor_application_modal" />
        <vendor-rejected-modal v-if="showVendorRejectedModal" ref="vendor_rejected_modal" />
    </div>
</template>
...
const VendorApplicationModal = defineAsyncComponent(() => import('@template1/vendor/components/modules/vendor/VendorApplicationModal.vue'));
const VendorPendingVendorModal = defineAsyncComponent(() => import('./VendorPendingVendorModal.vue'));
const VendorRejectedModal = defineAsyncComponent(() => import('@template1/vendor/components/modules/vendor/VendorRejectedModal.vue'));
...
import PsLazyImage from '@template1/vendor/components/core/image/PsLazyImage.vue'
    ...
    export default defineComponent({
        ...
        setup(props){
            ...
            const showVendorApplicationModal = ref(false);
            const showVendorPendingModal = ref(false);
            const showVendorRejectedModal = ref(false);
            ...
            async function clickVendor(status, id) {
                if(status == 0){
                    showVendorPendingModal.value = true;
                    await PsUtils.waitingComponent(vendor_pending_vendor_modal);
                    vendor_pending_vendor_modal.value.openModal();
                }else if(status == 1){
                    // vendor_application_modal.value.openModal();
                    showVendorRejectedModal.value = true;
                    await PsUtils.waitingComponent(vendor_rejected_modal);
                    vendor_rejected_modal.value.openModal(async() => {
                        showVendorApplicationModal.value = true;
                        await PsUtils.waitingComponent(vendor_application_modal);
                        vendor_application_modal.value.openModal();
                    });
                }else{
            ...
            return {
                ...
                PsConst,
                loginUserId,
                showVendorApplicationModal,
                showVendorRejectedModal,
