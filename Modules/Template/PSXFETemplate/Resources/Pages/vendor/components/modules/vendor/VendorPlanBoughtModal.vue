       ...
       <ps-modal ref="psmodal"  line="hidden" maxWidth="534px" :isClickOut='false' class='h-full  ' bodyHeight="max-h-75" theme="px-4 py-8 sm:p-8 rounded-lg">
        ...
        <template #body>
            ...
            <div class="grid grid-cols-3 sm:grid-cols-3 gap-4 mt-6">
                ...
                <ps-button v-show="false" colors="bg-transparent dark:bg-feSecondary-50 h-16" v-if="appInfoStore.appInfo.data?.offlineEnabled == '1'" class="" border="border hover:shadow" hover="none" padding="px-8 py-4" rounded="rounded-lg" @click="paymentClicked('OFFLINE')" >
...
export default defineComponent({
   ...
   setup(_,{ emit }) {
        ...
        async function openModal(vId,plan) {
            ...
            currencyShortForm.value = subScriptionPlan.value.paymentAttributes.currencyShortForm;
        ...
        async function razorClicked() {
            ...
            const options =
            {
                ...
                "amount": parseInt(price.value) * 100,
        ...
        async function flutterwaveClicked() {
            if(appInfoStore.appInfo.data?.flutterwavePublicKey == ""){
                ...
                console.log(subScriptionPlan.value.paymentAttributes.currencyShortForm);
                console.log(subScriptionPlan.value.paymentAttributes);

                useFlutterwave({
                    amount: parseInt(price.value.toString()),//amount
                    callback(data: any) {
                    ...
                    country: "US",
                    currency: subScriptionPlan.value.paymentAttributes.currencyShortForm,
