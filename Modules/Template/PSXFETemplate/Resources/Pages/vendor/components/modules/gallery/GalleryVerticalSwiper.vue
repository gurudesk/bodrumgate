<template>
  <div class="wrapper relative rounded-lg overflow-hidden"  >
    <div class="h-[400px] border rounded-lg" dir="ltr">
        <Splide class="z-0"
            ...
            <SplideSlide v-for="(gallery, index) in galleryList" :key="index">
                    <div class="w-full h-full relative bg-feSecondary-50">
                        ...
                        <ps-lazy-image
                            v-if="gallery.imgType == 'item'"
                            @click="openImage(gallery.imgType, gallery.imgPath, gallery.imgId, video)"
                            class="h-full w-full mx-auto object-cover overflow-hidden"
                            :src="$page.props.uploadUrl + '/' + gallery.imgPath"
                            :srcPlaceholder="$page.props.thumb1xUrl + '/' + gallery.imgPath"
        ...
        <Splide
            ...
            <SplideSlide v-for="gallery in galleryList" :key="gallery.imgId">
                <div class="w-full h-full">
                    <img v-if="gallery.imgType != 'item-video-icon'" class="w-full h-full object-cover rounded-md" v-lazy=" { src: $page.props.uploadUrl + '/' + gallery.imgPath, loading: $page.props.sysImageUrl+'/loading_gif.gif', error: $page.props.sysImageUrl+'/default_photo.png' }" />
                    <img v-else class="w-full h-full object-cover rounded-md" v-lazy=" { src: $page.props.uploadUrl + '/' + gallery.imgPath, loading: $page.props.sysImageUrl+'/loading_gif.gif', error: $page.props.sysImageUrl+'/default_photo.png' }" />
