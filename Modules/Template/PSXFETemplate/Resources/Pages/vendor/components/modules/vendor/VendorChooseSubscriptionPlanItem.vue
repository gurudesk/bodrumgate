<template>
    <div :class="{ 'border-fePrimary-500 h-90 lg:h-100': isHover , 'h-90' : !isHover}" class="w-full rounded overflow-hidden shadow-lg border border-inherit bg-feAchromatic-50 dark:bg-feAchromatic-800 cursor-pointer"
    ...
    <div  class="mt-8 w-full flex justify-center items-center flex-col mb-10">
        <ps-label :class="{ 'pt-0 lg:pt-5': isHover }" textColor="text-feSecondary-800 dark:text-feAchromatic-50 text-3xl font-semibold pb-3">{{plan ? plan.paymentInfo.value : ''}}</ps-label>
