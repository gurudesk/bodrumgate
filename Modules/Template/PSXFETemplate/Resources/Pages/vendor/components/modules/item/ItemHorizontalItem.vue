<template>
        ...
        <ps-card class="flex w-full bg-feAchromatic-50 flex-col lg:rounded-lg shadow-sm "
            ...
            <div class="lg:px-0 lg:py-0 px-1 "
                ...
                <div class="h-44 relative z-0">
                    ...
                    <ps-route-link :to="{ name: 'fe_item_detail', query: { item_id: item.id } }" class="absolute z-10">
                            ...
                            <div class="overflow-hidden h-44 w-full">
                                ...
                                <ps-lazy-image
                                    class="overflow-hidden transform transition duration-500 hover:scale-110 rounded-t-lg object-cover  w-screen h-44"
                                    :src="$page.props.thumb2xUrl + '/' + item.defaultPhoto.imgPath"
                                    :srcPlaceholder="$page.props.thumb1xUrl + '/' + item.defaultPhoto.imgPath"
                    ...
                    <div class="absolute left-2 bottom-2 z-20">
                        <ps-label v-if="item && item.isSoldOut == '1' && appInfoStore?.isSoldOutFeatureSettingOn()"
                ...
                <ps-route-link :to="{ name: 'fe_item_detail', query: { item_id: item.id } }">
                    ...
                    <div v-if="appInfoStore.appInfo.data?.psAppSetting?.hidePricesetting == PsConst.ONE && loginUserId == 'nologinuser'">
                        <ps-label textColor="font-semibold text-base text-fePrimary-500 ">
                            {{ item.itemCurrency.currencySymbol }} ****</ps-label>
                    </div>
                    <div v-else>
            ...
            <ps-route-link :to="{ name: 'fe_vendor_info', params: { vendorId: item?.vendor?.id }}"  v-if="appInfoStore?.isVendorSettingOn() && item?.vendor?.id != ''" >
                <div v-if="showProfile" class="pt-1 pb-2 flex items-center justify-between leading-none px-2  rounded-b-xl lg:rounded-b-2xl">
                    <div class="flex flex-row relative items-center no-underline text-feAchromatic-900">
                        ...
                        <ps-lazy-image
                            class="rounded-full bg-transparent lg:w-8 lg:h-8 w-6 h-6 flex items-center justify-center object-cover"
                            :src="$page.props.thumb1xUrl + '/' + item?.vendor?.logo?.imgPath"
                            :srcPlaceholder="$page.props.thumb1xUrl + '/' + item?.vendor?.logo?.imgPath"
            ...
            <ps-route-link v-else :to="{ name: 'fe_other_profile', params: { userId: item?.user?.userId }}" >
                <div v-if="showProfile" class="pt-1 pb-2 flex items-center justify-between leading-none px-2  rounded-b-xl lg:rounded-b-2xl">
                    <div class="flex flex-row relative items-center no-underline text-feAchromatic-900">
                        ...
                        <ps-lazy-image
                            class="rounded-full bg-transparent lg:w-8 lg:h-8 w-6 h-6 flex items-center justify-center object-cover"
                            :src="$page.props.thumb1xUrl + '/' + item?.user?.userCoverPhoto"
                            :srcPlaceholder="$page.props.thumb1xUrl + '/' + item?.user?.userCoverPhoto"
    ...
    <ps-loading-dialog v-if="showLoadingDialog" ref="ps_loading_dialog" />
</template>
    ...
    setup(props) {
        ...
        async function favouriteClicked(itemId) {
            if (loginUserId && loginUserId != PsConst.NO_LOGIN_USER) {
                ...
                await PsUtils.waitingComponent(ps_loading_dialog);
