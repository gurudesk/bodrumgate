...
class MPCTemplate1Controller extends Controller
{
    ...
    const ShoppingCartPage = self::parentPath."add_to_cart/ShoppingCart";
    const dataDeletionRequestFormPage = self::parentPath."user/DataDeletion";
    const orderHistoryPage = self::parentPath."order/list/OrderList";
    const orderDetailPage = self::parentPath."order/detail/OrderDetail";
    ...
    public function __construct(AboutService $aboutService,BlogService $blogService,PushNotificationMessageService $pushnotificationMessageService,LandingPageService $landingPageService, ItemService $itemService, CartService $cartService, ImageService $imageService, CategoryService $categoryService, UserService $userService)
    {
    ...
    public function checkVendor(Request $request)
    {
        ...
        $vendor = Vendor::where('owner_user_id', $user->id)
        ->where('status', 2)
        ->first();
        // Return a JSON response indicating whether the user is a vendor
        return response()->json(['vendorExists' => $vendor !== null]);
    }
    ...
    public function dataDeletionRequestForm()
    {
        if(!Auth::user()){
            return redirect()->route('login');
        }
        return renderView(self::dataDeletionRequestFormPage);
    }
    ...
    public function feOrderHistory(Request $request)
    {
        //for meta
        $this->setMeta($this->metaTitle ?? __('site_name'), $this->metaDescription ?? null, null);
        ...
        return renderView(self::orderHistoryPage);
    }

    public function feOrderHistoryDetail(Request $request)
    {
        //for meta
        $this->setMeta($this->metaTitle ?? __('site_name'), $this->metaDescription ?? null, null);
        ...
        return renderView(self::orderDetailPage, $dataArr);
    }

    public function feAddToCart(Request $request)
    {
        $cartData = $this->cartService->feAddToCart($request);
        return $cartData;
    }

    public function feShoppingCart(Request $request)
    {
        $shoppingCart = $this->cartService->feShoppingCart($request);
        //return $shoppingCart;
        return renderView(self::ShoppingCartPage, json_decode($shoppingCart, true) );
