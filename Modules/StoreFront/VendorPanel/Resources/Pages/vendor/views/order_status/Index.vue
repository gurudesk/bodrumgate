<template>
    ...
    <Head :title="$t('core__vendor_order_status_module')" />
    <ps-layout>
        ...
        <ps-label class="text-xl font-semibold" >{{ $t('core__vendor_order_status_module') }}</ps-label>
        ...
        <!-- data table start -->
        <ps-table2 :row="row" :search="search" :object="this.orderStatuses"
            ...
            <template #button>
                <ps-button v-if="storeCan.createOrderStatus" @click="handleCreate()" rounded="rounded-lg" type="button"
                    class="flex flex-wrap items-center"> <ps-icon name="plus" class="me-1 font-semibold" /> {{
                        $t('core_vendor__add_order_status') }}</ps-button>
            </template>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        console.log(props.orderStatuses);
        ...
        function handleSearchingSorting(page = null, row = null) {
            router.get(route('vendor_order_status.index'),
                {
                    sort_field: sort_field.value,
                    sort_order: sort_order.value,
                    page: page ?? props.orderStatuses.meta.current_page,
                    row: row ?? props.orderStatuses.meta.per_page,
                    search: search.value,
                },
                {
                    preserveScroll: true,
        ...
        function confirmDeleteClicked(id, vendor_id) {
            ps_danger_dialog.value.openModal(
                ...
                trans('core_vendor__delete_order_status_info'),
                trans('core__be_btn_confirm'),
                trans('core__be_btn_cancel'),
                () => {
                    router.delete(route("vendor_order_status.destroy", [vendor_id, id]), {
                        onSuccess: () => {
                            visible.value = true;
                            setTimeout(() => {
                                visible.value = false;
    ...
    methods: {
        ...
        handleEdit(vendor_id, id) {
            this.$inertia.get(route('vendor_order_status.edit', [vendor_id, id]));
        },
        handlePublish(row,hasPermission) {
            if(hasPermission && row.is_ps_default){
                this.$inertia.put(route('vendor_order_status.statusChange', [row.vendor_id, row.id]));
            }
